<template>
  <div>
    <div class="col-lg-12">
      ...
      <div>
        <select v-model="condition" class="form-group">
          <option value="" selected disabled hidden>검색 종류</option>
          <option v-for="(item, idx) in conditions" v-bind:value="item" v-bind:key="idx">{{ item }}</option>
        </select>
        <input type="text" id="low" ref="low" v-model="low" />
        <input type="text" id="high" ref="high" v-model="high" /> (1000원 단위)
        <button @click="dongChange(1)">검색</button>
      </div>
          ...
          <table class="table table-hover">
            <thead>
              <tr>
                ...
                <th class="text-center d-none d-md-table-cell">거래가격</th>
                <th class="text-center d-none d-md-table-cell">면적</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, index) in items" v-bind:key="index">
                ...
                <td class="text-center d-none d-md-table-cell">{{ item.dealAmount }}</td>
                <td class="text-center d-none d-md-table-cell">{{ item.area }}</td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  created() {
    this.addScript();
    axios
      .post("http://localhost:9999/happyhouse/houseinfo/PriceRangebyDong/" + 1, {
        high: this.high,
        low: this.low,
        search_value: this.dong,
      })
      .then(({ data }) => {
  ...
  methods: {
    ...
    dongChange(pageno) {
      ...
      if (this.condition == "면적") {
        axios
          .post("http://localhost:9999/happyhouse/houseinfo/AreaRangebyDong/" + pageno, {
            high: this.high,
            low: this.low,
            search_value: this.dong,
          })
        ...
        axios
          .post("http://localhost:9999/happyhouse/houseinfo/PriceRangebyDong/" + pageno, {
            high: this.high,
            low: this.low,
            search_value: this.dong,
          })
